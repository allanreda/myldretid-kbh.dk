...
class CloudUtils:
    def __init__(self, bq_client, storage_client, sm_client):
    ...
    def get_secret(self, secret_id, project_id):
        try:
            ...
            secret_name = self.sm_client.secret_path(project_id, secret_id) + '/versions/1'

            # Access the secret's value
            secret_string = self.sm_client.access_secret_version(name=secret_name).payload.data.decode('utf-8')
